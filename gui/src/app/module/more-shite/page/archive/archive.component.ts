...
export class ArchiveComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private apiClient: CommunityAPIClient,
  ...
  load() {
    this.apiClient
      .listArchive({ pageSize: PAGE_SIZE, page: this.currentPage })
      .pipe(takeUntil(this.destroy$))
      .subscribe((res: RskArchiveList): void => {
        this.archive = res.items;
        let totalPages: number = Math.ceil(res.resultCount / PAGE_SIZE);
        this.pages = Array(Math.min(totalPages, MAX_PAGINATION_LINKS))
          .fill(0)
          .map((x, i) => i + 1);
