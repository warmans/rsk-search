// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        (unknown)
// source: radio.proto

//go:build protoopaque

package api

import (
	_ "github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-openapiv2/options"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	emptypb "google.golang.org/protobuf/types/known/emptypb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type RadioState struct {
	state                         protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_CurrentEpisode     *CurrentRadioEpisode   `protobuf:"bytes,1,opt,name=current_episode,json=currentEpisode,proto3"`
	xxx_hidden_CurrentTimestampMs int32                  `protobuf:"varint,3,opt,name=current_timestamp_ms,json=currentTimestampMs,proto3"`
	unknownFields                 protoimpl.UnknownFields
	sizeCache                     protoimpl.SizeCache
}

func (x *RadioState) Reset() {
	*x = RadioState{}
	mi := &file_radio_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RadioState) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RadioState) ProtoMessage() {}

func (x *RadioState) ProtoReflect() protoreflect.Message {
	mi := &file_radio_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *RadioState) GetCurrentEpisode() *CurrentRadioEpisode {
	if x != nil {
		return x.xxx_hidden_CurrentEpisode
	}
	return nil
}

func (x *RadioState) GetCurrentTimestampMs() int32 {
	if x != nil {
		return x.xxx_hidden_CurrentTimestampMs
	}
	return 0
}

func (x *RadioState) SetCurrentEpisode(v *CurrentRadioEpisode) {
	x.xxx_hidden_CurrentEpisode = v
}

func (x *RadioState) SetCurrentTimestampMs(v int32) {
	x.xxx_hidden_CurrentTimestampMs = v
}

func (x *RadioState) HasCurrentEpisode() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_CurrentEpisode != nil
}

func (x *RadioState) ClearCurrentEpisode() {
	x.xxx_hidden_CurrentEpisode = nil
}

type RadioState_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	CurrentEpisode     *CurrentRadioEpisode
	CurrentTimestampMs int32
}

func (b0 RadioState_builder) Build() *RadioState {
	m0 := &RadioState{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_CurrentEpisode = b.CurrentEpisode
	x.xxx_hidden_CurrentTimestampMs = b.CurrentTimestampMs
	return m0
}

type CurrentRadioEpisode struct {
	state                protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_ShortId   string                 `protobuf:"bytes,1,opt,name=short_id,json=shortId,proto3"`
	xxx_hidden_StartedAt string                 `protobuf:"bytes,2,opt,name=started_at,json=startedAt,proto3"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *CurrentRadioEpisode) Reset() {
	*x = CurrentRadioEpisode{}
	mi := &file_radio_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CurrentRadioEpisode) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CurrentRadioEpisode) ProtoMessage() {}

func (x *CurrentRadioEpisode) ProtoReflect() protoreflect.Message {
	mi := &file_radio_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *CurrentRadioEpisode) GetShortId() string {
	if x != nil {
		return x.xxx_hidden_ShortId
	}
	return ""
}

func (x *CurrentRadioEpisode) GetStartedAt() string {
	if x != nil {
		return x.xxx_hidden_StartedAt
	}
	return ""
}

func (x *CurrentRadioEpisode) SetShortId(v string) {
	x.xxx_hidden_ShortId = v
}

func (x *CurrentRadioEpisode) SetStartedAt(v string) {
	x.xxx_hidden_StartedAt = v
}

type CurrentRadioEpisode_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	ShortId   string
	StartedAt string
}

func (b0 CurrentRadioEpisode_builder) Build() *CurrentRadioEpisode {
	m0 := &CurrentRadioEpisode{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ShortId = b.ShortId
	x.xxx_hidden_StartedAt = b.StartedAt
	return m0
}

type PutRadioStateRequest struct {
	state                         protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_CurrentEpisode     *CurrentRadioEpisode   `protobuf:"bytes,1,opt,name=current_episode,json=currentEpisode,proto3"`
	xxx_hidden_CurrentTimestampMs int32                  `protobuf:"varint,3,opt,name=current_timestamp_ms,json=currentTimestampMs,proto3"`
	unknownFields                 protoimpl.UnknownFields
	sizeCache                     protoimpl.SizeCache
}

func (x *PutRadioStateRequest) Reset() {
	*x = PutRadioStateRequest{}
	mi := &file_radio_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PutRadioStateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PutRadioStateRequest) ProtoMessage() {}

func (x *PutRadioStateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_radio_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *PutRadioStateRequest) GetCurrentEpisode() *CurrentRadioEpisode {
	if x != nil {
		return x.xxx_hidden_CurrentEpisode
	}
	return nil
}

func (x *PutRadioStateRequest) GetCurrentTimestampMs() int32 {
	if x != nil {
		return x.xxx_hidden_CurrentTimestampMs
	}
	return 0
}

func (x *PutRadioStateRequest) SetCurrentEpisode(v *CurrentRadioEpisode) {
	x.xxx_hidden_CurrentEpisode = v
}

func (x *PutRadioStateRequest) SetCurrentTimestampMs(v int32) {
	x.xxx_hidden_CurrentTimestampMs = v
}

func (x *PutRadioStateRequest) HasCurrentEpisode() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_CurrentEpisode != nil
}

func (x *PutRadioStateRequest) ClearCurrentEpisode() {
	x.xxx_hidden_CurrentEpisode = nil
}

type PutRadioStateRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	CurrentEpisode     *CurrentRadioEpisode
	CurrentTimestampMs int32
}

func (b0 PutRadioStateRequest_builder) Build() *PutRadioStateRequest {
	m0 := &PutRadioStateRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_CurrentEpisode = b.CurrentEpisode
	x.xxx_hidden_CurrentTimestampMs = b.CurrentTimestampMs
	return m0
}

type NextRadioEpisode struct {
	state              protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_ShortId string                 `protobuf:"bytes,1,opt,name=short_id,json=shortId,proto3"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *NextRadioEpisode) Reset() {
	*x = NextRadioEpisode{}
	mi := &file_radio_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NextRadioEpisode) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NextRadioEpisode) ProtoMessage() {}

func (x *NextRadioEpisode) ProtoReflect() protoreflect.Message {
	mi := &file_radio_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *NextRadioEpisode) GetShortId() string {
	if x != nil {
		return x.xxx_hidden_ShortId
	}
	return ""
}

func (x *NextRadioEpisode) SetShortId(v string) {
	x.xxx_hidden_ShortId = v
}

type NextRadioEpisode_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	ShortId string
}

func (b0 NextRadioEpisode_builder) Build() *NextRadioEpisode {
	m0 := &NextRadioEpisode{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ShortId = b.ShortId
	return m0
}

var File_radio_proto protoreflect.FileDescriptor

const file_radio_proto_rawDesc = "" +
	"\n" +
	"\vradio.proto\x12\x03rsk\x1a\x1cgoogle/api/annotations.proto\x1a.protoc-gen-openapiv2/options/annotations.proto\x1a\x1bgoogle/protobuf/empty.proto\"\x81\x01\n" +
	"\n" +
	"RadioState\x12A\n" +
	"\x0fcurrent_episode\x18\x01 \x01(\v2\x18.rsk.CurrentRadioEpisodeR\x0ecurrentEpisode\x120\n" +
	"\x14current_timestamp_ms\x18\x03 \x01(\x05R\x12currentTimestampMs\"O\n" +
	"\x13CurrentRadioEpisode\x12\x19\n" +
	"\bshort_id\x18\x01 \x01(\tR\ashortId\x12\x1d\n" +
	"\n" +
	"started_at\x18\x02 \x01(\tR\tstartedAt\"\x8b\x01\n" +
	"\x14PutRadioStateRequest\x12A\n" +
	"\x0fcurrent_episode\x18\x01 \x01(\v2\x18.rsk.CurrentRadioEpisodeR\x0ecurrentEpisode\x120\n" +
	"\x14current_timestamp_ms\x18\x03 \x01(\x05R\x12currentTimestampMs\"-\n" +
	"\x10NextRadioEpisode\x12\x19\n" +
	"\bshort_id\x18\x01 \x01(\tR\ashortId2\xb7\x03\n" +
	"\fRadioService\x12\x89\x01\n" +
	"\rGetRadioState\x12\x16.google.protobuf.Empty\x1a\x0f.rsk.RadioState\"O\x92A4\n" +
	"\x06search\x12 Get the current episode to play.*\bgetState\x82\xd3\xe4\x93\x02\x12\x12\x10/api/radio/state\x12\x89\x01\n" +
	"\fGetRadioNext\x12\x16.google.protobuf.Empty\x1a\x15.rsk.NextRadioEpisode\"J\x92A0\n" +
	"\x06search\x12\x1dGet the next episode to play.*\agetNext\x82\xd3\xe4\x93\x02\x11\x12\x0f/api/radio/next\x12\x8e\x01\n" +
	"\rPutRadioState\x12\x19.rsk.PutRadioStateRequest\x1a\x16.google.protobuf.Empty\"J\x92A,\n" +
	"\x06search\x12\x18Store the current state.*\bputState\x82\xd3\xe4\x93\x02\x15:\x01*\x1a\x10/api/radio/stateB\x88\x01\x92AW\x12\x052\x031.0*\x01\x01rK\n" +
	"2Radio service provides endpoints for radio player.\x12\x15https://scrimpton.comZ,github.com/warmans/rsk-search/server/gen/apib\x06proto3"

var file_radio_proto_msgTypes = make([]protoimpl.MessageInfo, 4)
var file_radio_proto_goTypes = []any{
	(*RadioState)(nil),           // 0: rsk.RadioState
	(*CurrentRadioEpisode)(nil),  // 1: rsk.CurrentRadioEpisode
	(*PutRadioStateRequest)(nil), // 2: rsk.PutRadioStateRequest
	(*NextRadioEpisode)(nil),     // 3: rsk.NextRadioEpisode
	(*emptypb.Empty)(nil),        // 4: google.protobuf.Empty
}
var file_radio_proto_depIdxs = []int32{
	1, // 0: rsk.RadioState.current_episode:type_name -> rsk.CurrentRadioEpisode
	1, // 1: rsk.PutRadioStateRequest.current_episode:type_name -> rsk.CurrentRadioEpisode
	4, // 2: rsk.RadioService.GetRadioState:input_type -> google.protobuf.Empty
	4, // 3: rsk.RadioService.GetRadioNext:input_type -> google.protobuf.Empty
	2, // 4: rsk.RadioService.PutRadioState:input_type -> rsk.PutRadioStateRequest
	0, // 5: rsk.RadioService.GetRadioState:output_type -> rsk.RadioState
	3, // 6: rsk.RadioService.GetRadioNext:output_type -> rsk.NextRadioEpisode
	4, // 7: rsk.RadioService.PutRadioState:output_type -> google.protobuf.Empty
	5, // [5:8] is the sub-list for method output_type
	2, // [2:5] is the sub-list for method input_type
	2, // [2:2] is the sub-list for extension type_name
	2, // [2:2] is the sub-list for extension extendee
	0, // [0:2] is the sub-list for field type_name
}

func init() { file_radio_proto_init() }
func file_radio_proto_init() {
	if File_radio_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_radio_proto_rawDesc), len(file_radio_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   4,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_radio_proto_goTypes,
		DependencyIndexes: file_radio_proto_depIdxs,
		MessageInfos:      file_radio_proto_msgTypes,
	}.Build()
	File_radio_proto = out.File
	file_radio_proto_goTypes = nil
	file_radio_proto_depIdxs = nil
}
